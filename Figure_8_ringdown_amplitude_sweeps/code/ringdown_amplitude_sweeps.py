...
MAX_WORKERS = 70
root = "../plots/ringdowns_dual_pulse_layout"
...
ax_ring.set_ylabel(r"$\omega_d$ [GHz]")
...
# panel tag
ax_ring.text(-0.06, 1.05, r"\textbf{a}", transform=ax_ring.transAxes,
...
ax_amp.set_ylabel("$A_1$")
add_cbar(im_amp, ax_amp, r"$\langle \sigma^{+}\sigma^{-} \rangle$")

ax_amp.text(
    0.98, 0.05, fr"$\omega_d = {FREQ_STAR:.2f}\,\mathrm{{GHz}}$",
    transform=ax_amp.transAxes,
    ha="right", va="bottom",
    bbox=dict(boxstyle="round,pad=0.25", fc="white", ec="none", alpha=0.8)
)
ax_amp.text(-0.06, 1.05, r"\textbf{b}", transform=ax_amp.transAxes,
            ha="left", va="bottom", fontsize=30, fontweight="bold")

# ───────────── 3 ▪ two-pulse sweep ─────────────
print("▸ Second-pulse amplitude sweep …")
pop_double = np.zeros((len(AMP2_SWEEP), len(tlist)))
with ProcessPoolExecutor(min(MAX_WORKERS, len(AMP2_SWEEP))) as pool:
    futures = {pool.submit(
        run_simulation_double_pulse,
        FREQ_STAR, AMP1_FIXED, amp2,
        PULSE1_NS, GAP_NS, PULSE2_NS,
        tlist, init_freqs, H_int,
        GAMMA, GAMMA_PHI
    ): i for i, amp2 in enumerate(AMP2_SWEEP)}
    for fut in tqdm(as_completed(futures), total=len(futures)):
        pop_double[futures[fut]] = fut.result()

im_d = ax_double.imshow(
    pop_double, origin="lower", aspect="auto",
    extent=[tlist[0]/μs, tlist[-1]/μs, AMP2_SWEEP[0], AMP2_SWEEP[-1]],
    cmap="inferno"
)
# pulse guides
...
ax_double.set_ylabel("$A_2$")
add_cbar(im_d, ax_double, r"$\langle \sigma^{+}\sigma^{-} \rangle$")

ax_double.text(
    0.95, 0.05,
    fr"$\omega_d = {FREQ_STAR:.2f}\,\mathrm{{GHz}}$"
    "\n"
    fr"$A_1 = {AMP1_FIXED:.2f}$",
    transform=ax_double.transAxes,
    ha="right", va="bottom",
    bbox=dict(boxstyle="round,pad=0.25", fc="white", ec="none", alpha=0.8)
)
ax_double.text(-0.06, 1.05, r"\textbf{c}", transform=ax_double.transAxes,
               ha="left", va="bottom", fontsize=30, fontweight="bold")
