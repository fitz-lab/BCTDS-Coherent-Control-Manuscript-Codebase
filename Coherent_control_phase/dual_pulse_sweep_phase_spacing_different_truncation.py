...
gs = gridspec.GridSpec(
    4, 7,
    width_ratios=[5, 0.3, 0.2, 3, 5, 0.3, 0.2],  # [plot1, gap, cbar2, gap, plot2, gap, cbar2]
...
# ax.set_ylabel("Frequency [GHz]", labelpad=7)
# # ax.set_xlim([0,1])
...
x_axis = np.arange(mag_avg_log_matrix.shape[1])  # Data points index
y_axis = pulse_phase_list  # Ensure correct length
x_min, x_max = x_axis[0] / 552.96, x_axis[-1] / 552.96
im = ax.imshow(
    mag_avg_log_matrix,
    aspect='auto',
...
ax.text(abcd_x, abcd_y, r'\textbf{b}', transform=ax.transAxes,
             fontsize=abcd_size, fontweight='bold', va='top', ha='right', color='black')
ax.text(i_x, i_y, r'\textbf{i}', transform=ax.transAxes,
             fontsize=i_ii_size, fontweight='bold', va='top', ha='right', color=i_ii_color)
...
file_path = os.path.join(fig_save_dir, f"Fig_sweep_spacing_and_phase.png")
if save_fig:
