...
fft_view_MHz        = 100
MAX_WORKERS         = 70
root                = "../plots_v5/interaction_scan_operator"
...
for col, (title, Jval) in enumerate(J_levels.items()):
    ...
    with ProcessPoolExecutor(min(MAX_WORKERS, len(FREQ_AXIS))) as pool:
        ...
        for fut in tqdm(as_completed(futures), total=len(futures)):
            ...
            ### Same definition as for the two-spin case, but with the right collective operators.
            phi_mat[:, j] = (np.angle(Sp)).astype(np.float32)
            # phi_mat[:, j] = (np.angle(Sm)).astype(np.float32)
            # phi_mat[:, j] = (np.angle(Sp/Sm)).astype(np.float32)
            # phi_mat[:, j] = (np.angle(Sp) - np.angle(Sm)).astype(np.float32)
            # phi_mat[:, j] = (np.angle(Sp)*np.angle(Sm)).astype(np.float32)
            phi_mat[:, j] = ((phi_mat[:, j] + np.pi) % (2*np.pi) - np.pi).astype(np.float32)
    ...
    # fft_phi = np.abs(np.fft.rfft(phi_mat[mask_tail, :], axis=0)[keep_idx, :]) + 1e-12
